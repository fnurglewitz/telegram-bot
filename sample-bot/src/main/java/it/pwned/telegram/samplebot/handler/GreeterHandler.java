	...
	protected boolean processMessage(Message m) {
		...
		if (m.new_chat_participant != null) {
			...
			bot.submitToExecutor(() -> {
				bot.api.sendMessage(m.chat.id, String.format("Welcome, %s!", m.new_chat_participant.first_name), null,
		...
		if (m.left_chat_participant != null) {
			...
			bot.submitToExecutor(() -> {
				bot.api.sendMessage(m.chat.id, String.format("Goodbye, %s!", m.left_chat_participant.first_name), null,
						null, m.message_id, null);
			});
		}
